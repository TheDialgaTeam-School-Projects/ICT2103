    ...
    <div class="col-sm-9 text-left"> 
      <table class="display" id="data_table" style="width: 100%">
        ...
        <?php
            ...
            $sql = "SELECT bs_ID, bs.s_ID, s.s_ID, s.name, year, enrolled, last_edited_by FROM batch_school bs, school s WHERE bs.s_ID = s.s_ID";
            ...
            while($row = mysqli_fetch_array($result,MYSQLI_ASSOC))
            {
                ...
                {
                    echo '<td>';
                    echo '<a href="modify.php?from=enroll&table=school&id='.$row['bs_ID'].'">Modify</a>';
                    echo '/';
                    echo '<a href="delete.php?from=enroll&table=school&id='.$row['bs_ID'].'">Delete</a>';
                    echo '</td>';
                }
                    echo '</tr>';
            }
     ...
     <table class="display" id="data_table2" style="width: 100%">
        ...
        <tbody> 
        <?php
            $sql2 = "SELECT bc_ID, bc.c_ID, c.c_ID, c.name, year, enrolled, last_edited_by FROM batch_course bc, course c WHERE bc.c_ID = c.c_ID";
            ...
            while($row2 = mysqli_fetch_array($result2,MYSQLI_ASSOC))
            {
                ...
                {
                    echo '<td>';
                    echo '<a href="modify.php?from=enroll&table=course&id='.$row2['bc_ID'].'">Modify</a>';
                    echo '/';
                    echo '<a href="delete.php?from=enroll&table=course&id='.$row2['bc_ID'].'">Delete</a>';
                    echo '</td>';
                }
                    echo '</tr>';
            }
