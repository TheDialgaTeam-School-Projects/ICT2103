...
if($_SERVER["REQUEST_METHOD"] == "POST") { 
        ...
        $sql = "UPDATE graduate_employment SET employ_rate=".$_POST['employ_rate'].", monthly_base_pay=".$_POST['mbp'].",last_edited_by='".$_SESSION['login_user']."' WHERE ge_ID=".$id."";
     ...
     <table class="table">
        ...
        <tbody> 
            ...
            <tr>
                            ...
                            <?php
                            $sql2 = "SELECT school,course,year,employ_rate,monthly_base_pay FROM graduate_employment WHERE ge_ID=$id";
                            $result2 = mysqli_query($db,$sql2);
                            $row2 = mysqli_fetch_array($result2,MYSQLI_ASSOC);
                            echo '<th scope="row">'.$row2['school'].'</th>';
                            echo '<td>'.$row2['course'].'</td>';
                            echo '<td>'.$row2['year'].'</td>';
                            ?>
                
                <td>
                    <input type="text" class="form-control" id="employ_rate" name="employ_rate"  placeholder="Enter amount">
                </td>
                <td>
